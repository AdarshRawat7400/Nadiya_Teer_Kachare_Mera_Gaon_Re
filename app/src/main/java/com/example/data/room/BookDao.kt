...
interface BookDao {
    ...
    @Query("SELECT * FROM audio_state WHERE bookId = :bookId LIMIT 1")
    fun getAudioState(bookId: String): Flow<AudioStateEntity?>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun saveAudioState(state: AudioStateEntity)
    
    @Query("SELECT * FROM reading_history WHERE bookId = :bookId ORDER BY viewedAt DESC LIMIT 10")
    fun getReadingHistory(bookId: String): Flow<List<ReadingHistoryEntity>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun addReadingHistory(history: ReadingHistoryEntity)
}
